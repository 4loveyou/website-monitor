...
长期运行的网站信息监控系统（PRD v1.2）：每 30 分钟自动检查 Alpha123「今日空投」与「空投预告」两个板块，识别新增与修改并通过微信通知用户；删除只记录不通知。正式运行于免费云端，不依赖用户电脑常开、不依赖用户 VPN、不产生额外费用。

## 状态

- 版本：v0.4.1（今日空投 + 空投预告双板块；每 30 分钟；云端已上线）
...
- 每 30 分钟自动检查 https://alpha123.uk/zh/index.html 的「今日空投」+「空投预告」
...
说明：`run` 即“一轮检查”；每 30 分钟调度由云端（GitHub Actions）负责，
没有长期常驻进程。
...
4. 手动触发一次工作流验证初始化消息，之后每 30 分钟自动运行
